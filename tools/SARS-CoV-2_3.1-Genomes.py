...
def __main__():
    ...
    if args.library=='iont':
        ...
        # FILTER HUMAN GENOME
        subprocess.call("bowtie2 -p ${GALAXY_SLOTS:-4} -x '/ariesdb/database/covid19/Humangenome' -U trimmed1.fq --un filtered.fq --very-fast | samtools sort -@${GALAXY_SLOTS:-2} -O bam -o " + args.human_genome_aligned, shell=True)
    ...
    # Illumina
        ...
        subprocess.call("java ${_JAVA_OPTIONS:--Xmx8G} -jar trimmomatic.jar PE -threads ${GALAXY_SLOTS:-6} -phred33 " + args.input1 + " " + args.input2 + " trimmed1.fq fastq_out_r1_unpaired trimmed2.fq fastq_out_r2_unpaired LEADING:15 TRAILING:15 MINLEN:30 SLIDINGWINDOW:4:15", shell=True)
         # FILTER HUMAN GENOME
        subprocess.call("bowtie2 -p ${GALAXY_SLOTS:-4} -x '/ariesdb/database/covid19/Humangenome' -1 trimmed1.fq -2 trimmed2.fq  --un-conc filtered.fq --very-fast | samtools sort -@${GALAXY_SLOTS:-2} -O bam -o " + args.human_genome_aligned, shell=True)
        # ALIGN SARS-COV-2 GENOME
